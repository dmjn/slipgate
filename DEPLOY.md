...
Slipgate is a static site: one HTML file plus vendored libraries and icons. Any
host that serves files over HTTPS will do. HTTPS is required, since the service
worker and the clipboard both refuse to run without it.
...
See the README. Both vendored libraries ship their licence files alongside them
in `vendor/`.
